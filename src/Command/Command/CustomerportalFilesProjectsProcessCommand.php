...
class CustomerportalFilesProjectsProcessCommand extends Command
{
	public $hidden = true;
	public const string TYPE_CP_QUOTES_EXTRA_FILES = 'quotes';
	public const string TYPE_CP_PROJECT_EXTRA_FILES = 'projects';
	...
	{
		$this
			->setName('customerportal:files:projects:process')
			->addOption(
				'data',
				'd',
		...
		try {
			...
			switch ($object->EntityName) {
				case self::TYPE_CP_PROJECT_EXTRA_FILES:
					...
					$dBEntityObject = $this->projectRepo->find($object->entityId);
					break;
				case self::TYPE_CP_QUOTES_EXTRA_FILES:
					$entityObject = $this->xtrfCon->getQuote($object->entityId);
					$dBEntityObject = $this->quoteRepo->find($object->entityId);
	...
	private function updateTaskFiles(array $tokenList, array $taskList, object &$originalData, $entityId, string $entityType): void
	{
		...
		if (CustomerportalFilesProjectsProcessCommand::TYPE_CP_PROJECT_EXTRA_FILES === $entityType) {
		...
		if (CustomerportalFilesProjectsProcessCommand::TYPE_CP_QUOTES_EXTRA_FILES === $entityType) {
		...
		foreach ($uniqueData as $token) {
			...
			$this->redisClients->redisMainDB->zadd(RedisClients::SESSION_KEY_PENDING_FILES_ORDER, microtime(true), $token);
