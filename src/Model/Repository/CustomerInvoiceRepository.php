...
class CustomerInvoiceRepository extends EntityRepository
{
	...
	{
		$class = $em->getClassMetadata(CustomerInvoice::class);
	...
	public function getCountSearchInvoices(array $params): bool|int|string|null
	{
		$qb = $this->createQueryBuilder('inv');
	...
	private function invoiceSearchFilter(array $params, &$qb): void
	{
		...
		if (!empty($params['search']) || !empty($params['contact_person_id'])) {
			$qb
				...
				->leftJoin('accp.customer', 'cust')
				->leftJoin('accp.contactPerson', 'contp');
				...
				$qb
					->andWhere(
						$qb->expr()->orX(
							...
							$qb->expr()->like('LOWER(inv.customerName)', 'LOWER(:search)'),
							$qb->expr()->like('LOWER(cust.name)', 'LOWER(:search)')
						)
					)
		...
		if (!empty($params['customer_id'])) {
			$qb
				->andWhere(
					$qb->expr()->in('inv.customer', ':customerId')
				)
