...
namespace App\Workflow\Services\BlXtrf;
...
class Start implements WorkflowInterface
{
	...
	{
		$this->loggerSrv->addInfo('Starting BL-XTRF workflow.');
		...
		try {
			$registryWorkflow = $this->registry->get($history, 'wf_bl_xtrf');
			...
			if (!is_array($params['filters']['start_date'])) {
				$params['filters']['start_date'] = [
					$this->utilsSrv->getDateByFormat($params['filters']['start_date'])->format('d/m/Y'),
			...
			$this->loggerSrv->addError('Error while starting Bl-XTRF invoices workflow.', $thr);
		}
	}
	...
	private function valid(array $params): bool
	{
		...
		if (count($diff)) {
			foreach ($diff as $item) {
				$this->loggerSrv->addError("mandatory param $item is not defined into filters.");
			}
