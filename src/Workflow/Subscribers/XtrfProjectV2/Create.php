...
class Create implements EventSubscriberInterface
{
	...
	{
		return [
			'workflow.xtrf_project_v2.completed.process' => 'create',
		];
	}
	...
	{
		$this->loggerSrv->addInfo('Creating projects for XtrfProject Workflow');
		...
		try {
						...
						foreach ($context['files'] as $file) {
							...
							if (true === boolval($context['batch'])) {
								try {
									...
									$this->loggerSrv->addError("XTRF Project failed {$file['name']}", $thr);
									$context['template']['inputFiles'] = [];
									continue;
								}
							} else {
						...
						foreach ($context['files'] as $file) {
							...
							if (true === boolval($context['batch'])) {
								try {
									...
									$this->loggerSrv->addError("XTRF Quote failed {$file['name']}", $thr);
									$context['template']['inputFiles'] = [];
									continue;
								}
							}
			...
			if ($wf->can($history, 'create')) {
				$wf->apply($history, 'create');
			}
		} catch (\Throwable $thr) {
			$this->monitorLogSrv->appendError([
				'message' => 'Workflow finished with error on Creating Projects',
			]);
			$this->loggerSrv->addError('Workflow finished with error on Creating Projects', $thr);
			...
			switch ($context['type']) {
				...
				case Start::TYPE_QUOTE:
					...
					if (null !== $quote) {
						...
						$context['request']['links'][] = $context['linkUrl'].$quote->id;
