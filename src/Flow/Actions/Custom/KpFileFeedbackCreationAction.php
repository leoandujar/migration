...
class KpFileFeedbackCreationAction extends Action
{
	public const ACTION_DESCRIPTION = 'Create feedback file';
	...
	public function __construct(
		...
		$this->actionName = 'KpFileFeedbackCreationAction';
		$this->inputs = [];
	}
	...
	public function run(): string
	{
		...
		try {
			...
			$mapping = $context['files_languages'] ?? [];
			foreach ($context['filesUnZipped'] as $file) {
				$status = 'Ready';
				if (!isset($mapping[$file['name']])) {
					$status = 'Recived';
				}
				$filesStatusMapping[$file['name']] = [
					'status' => $status,
					'reason' => 'Good Record',
				];
			}
			...
			$devFolderName = ($context['testMode'] ?? null) ? $context['testingFolder'] : $context['productionFolder'];
